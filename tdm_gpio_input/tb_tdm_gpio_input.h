...
private:
  void updateClocks(int cur_clk_cycle) {
    if (cur_clk_cycle % CLKS_PER_BIT == 0) {
      sclk = 0;
    } else if (cur_clk_cycle % CLKS_PER_BIT == CLKS_PER_BIT / 2) {
      sclk = 1;
    }

    if (sample_index == NUM_CHANNELS - 1 && bit_index == CHANNEL_SIZE - 1) {
      lrclk = 1;
    } else {
      lrclk = 0;
    }
  };
  void processData(int cur_clk_cycle, ap_uint<32> &data) {
    // Process the data based on the state of data_lock.
    if (!lrclk_seen) {
      sdata = (bit_index + sample_index) % 2;
    } else {
      sdata = data[CHANNEL_SIZE - 1 - bit_index];
    }
  }
  void simulation_data(ap_uint<32> &data_container) {
    static bool rdy_for_next_sample = true;
    if (lrclk_seen) {
      if (!bit_index && rdy_for_next_sample) {
        // Here we can set any new sample
        data_container = (data_container + 1) | (ap_uint<32>)0;
...
public:
  void master_clk(int cur_clk_cycle, ap_uint<32> &data) override {
