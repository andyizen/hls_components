...
#pragma HLS INTERFACE mode = ap_ctrl_none port = return
  ...
  static Biquad_DFII_fix biquad = Biquad_DFII_fix(coeff, dly);
  ...
  if (_prcs_stt_ == READ) {
    ...
    if (_mem_stt_ == LOAD) {
      // Prepare Delays
      dly.w1 = mem_dly[_ch_cnt_ * NUM_DELAYS + 0];
      dly.w2 = mem_dly[_ch_cnt_ * NUM_DELAYS + 1];
    ...
    mem_dly[_ch_cnt_ * NUM_DELAYS + 0] = dly.w1;
    mem_dly[_ch_cnt_ * NUM_DELAYS + 1] = dly.w2;
